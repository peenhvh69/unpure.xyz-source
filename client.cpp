...
void Client::DrawHUD() {
	...
	std::stringstream ss;
	ss << tfm::format(XOR("Unpure.xyz | fps: %i | ms: %i"), fps, ms);
...
void Client::ClanTag()
{
	...
	std::string szClanTag = XOR("Unpure.xyz");
	...
	if (g_menu.main.misc.clantag.get()) {
		...
		if (g_csgo.m_gamerules->m_bFreezePeriod())
		{
			if (is_freeze_period)
			{
				SetClanTag("Unpure.xyz"); //Project-X
			}
		...
		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 2.9) % 14) {
			...
			case 2: {  SetClanTag("Un"); break; }
			case 3: {  SetClanTag("Unp"); break; }
			case 4: {  SetClanTag("Unpu"); break; }
			case 5: {  SetClanTag("Unpur"); break; }
			case 6: {  SetClanTag("Unpure"); break; }
			case 7: {  SetClanTag("Unpure."); break; }
			case 8: {  SetClanTag("Unpure.x"); break; }
			case 9: {  SetClanTag("Unpure.xy"); break; }
			case 10: { SetClanTag("Unpure.xyz"); break; }
			case 11: { SetClanTag("Unpure.xy"); break; }
			case 12: { SetClanTag("Unpure.x"); break; }
			case 13: { SetClanTag("Unpure."); break; }
			case 14: { SetClanTag("Unpure"); break; }
			case 15: { SetClanTag("Upure"); break; }
			case 16: { SetClanTag("Uure"); break; }
			case 17: { SetClanTag("Ure"); break; }
			case 18: { SetClanTag("Ue"); break; }
			case 19: { SetClanTag("Ue"); break; }
			case 20: { SetClanTag("U"); break; }
...
void Client::ClanTag2()
{
	...
	if (g_menu.main.misc.clantag2.get()) {
		...
		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 2.5) % 14) {
			...
			case 19: { SetClanTag("Unpure.xyz"); break; }
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::yellowgreen, XOR("[Unpure.xyz] "));
